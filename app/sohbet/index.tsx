/**
 * SOHBETLER — WhatsApp mantığında mesaj kutusu.
 *
...
export default function ChatInboxScreen() {
  ...
  const open = useCallback(
    (conversation: ChatConversation) => router.push(`/sohbet/${conversation.id}` as never),
    [router],
  );
  const compose = useCallback(() => router.push("/sohbet/yeni" as never), [router]);
  ...
  return (
    <SafeAreaView style={styles.screen} edges={["top"]}>
      <ScreenHeader
        title="Mesajlar"
        subtitle={unread > 0 ? `${unread} okunmamış mesaj` : "Yönetim, takımın ve oyuncular"}
        back
        scrollY={scrollY}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <EmptyState
              ...
              body={search ? "Başka bir ad dene." : "Yönetime, takım grubuna ya da bir oyuncuya ilk mesajı gönder."}
...
const ConversationRow = memo(function ConversationRow({
  ...
  const lastMine = conversation.last_message?.sender.is_me;
  return (
    <Touchable
      feedback="row"
      haptic="selection"
    ...
    >
      <View style={styles.avatarWrap}>
        ...
        <View style={[styles.typePip, conversation.type === "management" ? styles.typePipBrand : null]}>
...
const styles = StyleSheet.create({
  ...
  searchWrap: { paddingHorizontal: layout.screenPadding, paddingBottom: space.sm },
  skeleton: { paddingHorizontal: layout.screenPadding, paddingTop: space.sm },
  list: { flexGrow: 1, paddingBottom: layout.tabBarHeight + space.huge },
