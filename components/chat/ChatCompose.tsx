...
import { adminChat, getDirectory, openConversation, type ChatUser, type OpenConversationInput } from "@/lib/api/chat";
...
export function ChatCompose({ admin = false }: ChatComposeProps) {
  const auth = useAuth();
  const router = useRouter();
  const toast = useToast();
  const queryClient = useQueryClient();
  const [tab, setTab] = useState<Tab>("all");
  const TABS = admin ? ALL_TABS.filter((item) => item.key !== "management") : ALL_TABS;
  ...
  const query = useQuery({
    queryKey: [...queryKeys.chatDirectory(debounced), admin ? "admin" : "member"] as const,
    queryFn: () => (admin ? adminChat.getDirectory(debounced) : getDirectory(debounced)),
  ...
  const open = useMutation({
    mutationFn: (input: OpenConversationInput) => (admin ? adminChat.openConversation(input) : openConversation(input)),
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    if ((tab === "all" || tab === "team") && data.teams.length) {
      out.push({ kind: "section", key: "s-team", title: admin ? "Takım grupları" : "Takım grubum" });
    ...
    ({ item }: { item: Row }) => {
      ...
      return (
        <DirectoryRow
          ...
          onPress={() => (tab === "group" ? toggle(item.user.user_id) : open.mutate(admin ? { type: "member", user_id: item.user.user_id } : { type: "direct", user_id: item.user.user_id }))}
          disabled={open.isPending}
        />
      );
    },
  ...
  return (
    <SafeAreaView style={styles.screen} edges={["top", "bottom"]}>
      <ScreenHeader
        title="Mesaj oluştur"
        subtitle={admin ? "Kime yazacaksınız?" : "Kime yazmak istiyorsun?"}
        back
        bottom={
